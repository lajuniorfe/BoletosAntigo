...
namespace SdmBoletos.Controllers
{
    ...
    public class BoletosController : Controller
    {
        ...
        public IActionResult BuscarBoletos(string indicador, string teste, [FromBody] Dictionary<string, object> ParametrosRecebido)
        {
            try
            {
                ...
                if (teste == "s")
                {
                    ...
                    var boletosTeste = _Boletos.BuscarBoletosTeste(jsonTeste.TokenAcesso, indicador, ParametrosRecebido);
                    if (boletosTeste.Contains("erros"))
                    {
                        return BadRequest(boletosTeste);
                    }
                    else
                    {
        ...
        [HttpGet("testar")]
        public IActionResult PixTeste()
        {
            var chavepix = "asdasdas";
            var tt = _Boletos.PixTeste(chavepix);

            return new ObjectResult(tt);
        }
    }
}
